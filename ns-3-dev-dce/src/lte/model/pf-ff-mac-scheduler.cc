    ...
    m_nextRntiUl (0)
{
  ...
  //ljw 
  timeSeqLog = fopen("rbTimeSeq.txt", "w");
}
...
PfFfMacScheduler::~PfFfMacScheduler ()
{
  NS_LOG_FUNCTION (this);
   std::map <uint16_t, std::map<uint8_t, uint32_t> >::iterator it= m_rbFreqMaps.begin();
   std::vector<float> ha;
   std::vector<uint32_t> fre;
   std::vector<unsigned long> sumL;
   //fre.resize(m_rbFreqMaps.size());

      while (it!=m_rbFreqMaps.end ())
      {
          unsigned long sum = 0;
          unsigned long ws = 0;
          float avg = 0;
          std::cout<<"#UE :" <<it->first<<"**************"<<std::endl;
          std::map<uint8_t, uint32_t>& mapRef = it->second;
          std::map<uint8_t, uint32_t>::iterator itI = mapRef.begin();
          while(itI != mapRef.end())
          {
              std::cout<<int(itI->first)<<", "<<itI->second<<std::endl;
              sum += (itI->first) * (itI->second);
              ws += itI->second;
              itI++;
          }
          sumL.push_back(sum);
      ...
      for (unsigned int i=0; i< sumL.size(); i++)
          rSum += sumL[i];



      FILE* fp = fopen("rbResult.txt", "w");
    
      for (unsigned int i=0; i<sumL.size(); i++)
      {
        std::cout<<float(sumL[i]) / rSum * 100<<" ";
        fprintf(fp, "%.4f ", float(sumL[i]) / rSum * 100);
      }
      std::cout<<std::endl;
      fprintf(fp, "\n");
     ...
     for (unsigned int i=0; i<fre.size(); i++)
      {
        std::cout<<float(fre[i]) / freSum * 100<<" ";
        fprintf(fp, "%.4f ", float(fre[i]) / freSum * 100);
      }
      std::cout<<std::endl;
      fprintf(fp, "\n");
      ...
      for (unsigned int i=0; i<ha.size(); i++)
      {
        std::cout<<ha[i] *  (float(fre[i]) / freSum) <<" ";
        fprintf(fp, "%.4f ", ha[i] *  (float(fre[i]) / freSum));
      }
      std::cout<<std::endl;
      fprintf(fp, "\n");
...
PfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  NS_LOG_WARN(this << "**ljw** rbgSize=" << rbgSize << " rbgNum= " << rbgNum << "m_cschedCellConfig.m_dlBandwidth=" << m_cschedCellConfig.m_dlBandwidth);

  //ljw the best way to look at the allocation might be iterating through this map
  ...
  fprintf(timeSeqLog, "%.6f \t",  Simulator::Now ().GetNanoSeconds () / (double) 1e9);
  ...
  while (itMap != allocationMap.end ())
    {
      ...
      //this gives how many link layer download request received
      uint16_t lcActives = LcActivePerFlow ((*itMap).first);
      NS_LOG_WARN (this << "**ljw** Allocate user " << newEl.m_rnti << " rbg(lcActives) " << lcActives);
      if (lcActives == 0)
        {
      ...
      NS_LOG_WARN(this << "**ljw** Allocate user " << newEl.m_rnti << " RgbPerRnti " <<RgbPerRnti );

      std::map <uint8_t,uint32_t>::iterator itFreq;
      std::map <uint8_t,uint32_t> & mapRef = m_rbFreqMaps[newEl.m_rnti]; 
      itFreq = mapRef.find(RgbPerRnti*rbgSize);
      if(itFreq == mapRef.end())
      {
          mapRef[RgbPerRnti*rbgSize] =1;
      }
      else
      {
          itFreq->second +=1;
      ...
      fprintf(timeSeqLog, "(%d, %d)\t", newEl.m_rnti , RgbPerRnti*rbgSize);  
      ...
      for (uint8_t j = 0; j < nLayer; j++)
        {
          ...
          NS_LOG_WARN (this <<"**ljw**  rnti="<<newEl.m_rnti<<" tbSize based on table " << tbSize);
