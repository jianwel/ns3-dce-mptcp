...
def runOnce(nUE, D, dataFolder, fb, isFurther):
    ...
    runstr = randomString +"./waf --run \"oneNewLTE --nUE=%d "%nUE+ " --D="+str(D)+ " --isOneFurther=%d"%isFurther + " --B=%d"%(tcpR[nUE]*1600000) +" --disWifi=true --isUDP=1  --stopTime=" + str(udpStopTime)+" --ns3::ConfigStore::Filename=../../lte.default\"  | tee %s "%outfileName;
    print runstr
    os.system(runstr)
    ...
    os.system("./sumMUdp.py "+str(nUE) +" > iperfR.txt")
    os.system("./sumMTb.py "+str(nUE) +" > TbR.txt")
    os.system("./sumMSinr.py "+str(nUE) +" > SINRR.txt")
