...
#include "ns3/kernel-log-reader-helper.h"
...
//////////////////////////////
//need to write as a member function of node?
//or a single class that takes in a node container
//I decide to move it to a helper class in ns3-dce later, i will first test it quickly with one simple srrt reading
//return 0 if OK, return 1 if error
//the user need to init and release the resultArray

//FILE** kernelMessageFp = NULL;
...
{
    readerP->doOneRead();
    cout<<"####averge srtt"<< readerP->getMetric(0, "srrt")<<endl; 

  Simulator::Schedule (Seconds (INTERVAL), &PrintMetric); 
}
/*
...
//I was trying to use the following functions to read proc fs 
//but the dce implementation of fs is only partial
...
//change to another loc, need to change 6 times
   ...
   Simulator::Schedule (Seconds (next), &PrintLoc, &nodes, next);
...
uint routerI = 0;
  ...
  metrics.push_back("srtt");
  string pattern("ljw_print");

  KernelLogReaderHelper reader(metrics, &nodes, pattern);
  readerP = &reader;
//CreateObjectsWithAttributes
  ...
  if (!disLte)
    {
      ...
      setPos (enbNodes.Get (0), - LTE_D, 0, 0);
