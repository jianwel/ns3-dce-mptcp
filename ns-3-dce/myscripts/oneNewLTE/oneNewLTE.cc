...
#include "ns3/folder-create-helper.h"
...
int main (int argc, char *argv[])
{
    ...
    //cmd_oss << "tmp";
    //UtilsEnsureDirectoryExists (cmd_oss.str ());
    ...
    unsigned long int bandPre = 5000000;

    int isUDP =1;
    int iperfV = 2;

    int furtherDistance = 21000;
    int isOneFurther=0;
    ...
    cmd.AddValue ("isOneFurther", "put one UE further.", isOneFurther);
    ...
    cmd.AddValue ("D", "distance of UE to eNodeB.", distance);
    cmd.AddValue ("B", "-b option used in iperf.", bandPre);
    cmd.AddValue ("isUDP", "is using UDP or TCP.", isUDP);
    cmd.AddValue ("iperfV", "iperf version", iperfV);
 ...
 int dl_earfcn = 100;
    ...
    int runNumber = RngSeedManager::GetRun ();
    cout<<"RunNumber = "<< runNumber<<endl;



    //trying to match the sprint band 25

    //set to use 20MHz or not
   ...
   // Config::SetDefault ("ns3::WifiPhy::TxPowerLevels", StringValue ("7"));
    ...
    cout<<"lte dl frequency ="<< ns3::LteSpectrumValueHelper::GetCarrierFrequency(dl_earfcn)<<endl;
    cout<<"lte ul frequency ="<< ns3::LteSpectrumValueHelper::GetCarrierFrequency(ul_earfcn)<<endl;
    cout<<"lte downlink bandwith ="<< ns3::LteSpectrumValueHelper::GetChannelBandwidth(25)<<endl;

    disWifi = true;

    cout<<"D = "<<distance<<endl;
cout<<"iperfv ="<<iperfV<<endl;
cout<<"isUDP =" <<isUDP<<endl;

    cout<<"dis options "<<disLte <<" "<<disWifi<<endl;
    // disWifi = true;
...
*/
    ...
    /*
        for (int i=0; i< nodes.GetN(); i++)
          setPosSpeed((nodes.Get(i)), -distance, 0, 0, Vector(0, 0, 0) );
...
////////////////////////////////////////////////////////////////
 ...
 // if (ifTraceFile.good ())
      ...
      lteHelper->SetFadingModelAttribute ("TraceFilename", StringValue(traceName));
...
////////////////////////////////////////////////////////////////
    ...
    if (!disLte)
    {
      ...
      // cout<<"####Downlink RB "<<int(enbLteDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetDlBandwidth ())<<endl;
       enbLteDevs.Get(0)->GetObject<LteEnbNetDevice>()->SetDlEarfcn(dl_earfcn);
       enbLteDevs.Get(0)->GetObject<LteEnbNetDevice>()->SetUlEarfcn(ul_earfcn);
       //cout<<"##lte downlink bandwith ="<< ns3::LteSpectrumValueHelper::GetChannelBandwidth(enbLteDevs.Get(0).GetDlBandwidth ())<<endl;
       //
       //cout<<"lte dl frequency ="<< ns3::LteSpectrumValueHelper::GetCarrierFrequency(8665)<<endl;
      // cout<<"lte ul frequency ="<< ns3::LteSpectrumValueHelper::GetCarrierFrequency(26665)<<endl;
...
*/
    ...
    FolderCreateHelper dce;
    ApplicationContainer apps;

    dce.SetStackSize (1 << 20);
/*
        FolderCreateHelper fh;
        apps = fh.Install (nodes);
...
*/
    ...
    for(uint32_t u = 0; u < nodes.GetN (); ++u){
        ...
        {
        dce.ParseArguments ("-J");
        }
        dce.AddArgument ("-i");
        dce.AddArgument ("5");
        //dce.ParseArguments ("-l 1K");
...
*/
   ...
   // Simulator::Schedule (Seconds (5), &PrintLoc, &nodes, printTimer);
    ...
    csmahelper.EnablePcapAll ("csma-lte", false);
    csmahelperWifi.EnablePcapAll ("csma-wifi", false);
    ...
    Simulator::Stop (Seconds (stopTime+9));
    Simulator::Run ();
